...
public class NStackAppService : INStackAppService
{
    ...
    public async Task<DataMetaWrapper<TSection>> GetDefaultResourceAsync<TSection>(NStackPlatform platform, string version, string environment = "production", bool developmentEnvironment = false, bool productionEnvironment = true) where TSection : ResourceItem, new()
    {
    ...
    private async Task<DataMetaWrapper<TSection>> GetLocalizationAsync<TSection>(ResourceData localizeToFetch) where TSection : ResourceItem, new()
    {
